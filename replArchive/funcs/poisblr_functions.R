...
se_poisblr<-function(tab,Y,W,X,Z,calcSE=TRUE)
{
...
poisblr <- function(Y, W, X, Z, calcSE=TRUE){
 
  # get param estimates
  mod = poisblr_alsfit(Y,W,X,Z,trace=FALSE)
  ...
  # pred effs
  pred = exp(eta_tab(tab,W,X,Z) )
  ll = mll_poisblr(tab, Y, W, X, Z)  
