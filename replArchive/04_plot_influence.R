...
fname=paste0(gpth, 'matlConf_map.eps')
...
fNamesInflA = paste0(gpth, 'aInfl_', gsub('-','_',timeLabs[pds]), '.eps')
fNamesInflB = paste0(gpth, 'bInfl_', gsub('-','_',timeLabs[pds]), '.eps')
...
lapply(1:length(pds), function(i){
	...
	postscript(file=fNamesInflA[i], width=13, height=8, horizontal=FALSE, onefile=FALSE, paper="special")
	...
	postscript(file=fNamesInflB[i], width=13, height=8, horizontal=FALSE, onefile=FALSE, paper="special")
