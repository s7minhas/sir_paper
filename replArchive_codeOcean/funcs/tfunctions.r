...
amprod_dyn<-function(A,M,k)
{
  K<-length(dim(A))
  AM <- do.call('cbind', lapply(1:dim(A)[3], function(t){
    M[[t]] %*% mat(A[,,t,drop=FALSE],k) }))
  AMA<-array(AM, dim=c(dim(M[[1]])[1],dim(A)[-k]) )
  aperm(AMA,  match(1:K,c(k,(1:K)[-k]) ) )
}

tprod_dyn<-function(A,B,modes=1:length(B))
{
  X<-A
  for(k in modes) { X<-amprod_dyn(X,B[[k]],k) }
  X
}